...
class CardItem{
  ...
  CardItem({required this.id, required this.name, required this.description, required this.quantity, required this.price, required this.picPath});
...
class _BasketPageState extends State<BasketPage> {
  ...
  Future<List<CardItem>> fetchBasket() async {
    ...
    final response = await http.get(
      Uri.parse('$server/api/user/active_basket/$globalUserId'),
      headers: {
  ...
  Future<void> submitCard() async {
    final response = await http.get(
      Uri.parse('$server/api/user/submit_basket/$globalUserId'),
      headers: {
        'Content-Type': 'application/json'
      },
    );
    if (response.statusCode == 200){
      ...
      Flushbar(
        ...
        messageText: Text('خرید شما با موفقیت پرداخت شد',
          textAlign: TextAlign.center,
          style: TextStyle(
            color: Colors.white,
            fontWeight: FontWeight.bold,
  ...
  Future<void> cencelingCard() async {
    final response = await http.get(
      Uri.parse('$server/api/user/cancel_basket/$globalUserId'),
      headers: {
        'Content-Type': 'application/json'
      }
    );
    ...
    if(response.statusCode == 200){
      ...
      Flushbar(
        ...
        messageText: Text('خرید به درخواست شما کنسل شد',
          textAlign: TextAlign.center,
          style: TextStyle(
            color: Colors.white,
            fontWeight: FontWeight.bold,
  ...
  Future<void> increaseQuantity({ required userId, required productId}) async {
    final response = await http.get(
      Uri.parse('$server/api/user/$userId/products/$productId/add_to_card_from_card'),
      headers: {
        'Content-Type': 'application/json'
      }
    );
    ...
    throw Exception('خطا در انجام عملیات');
  }

  Future<void> decreaseQuantity({ required userId, required productId}) async {
    final response = await http.get(
        Uri.parse('$server/api/user/$userId/products/$productId/minus_to_card_from_card'),
        headers: {
          'Content-Type': 'application/json'
        }
    );
                                            ...
                                            Container(
                                                child: SizedBox(
                                                  ...
                                                  child: MaterialButton(
                                                      ...
                                                      onPressed: (){
                                                        increaseQuantity(productId: item.id, userId: globalUserId);
                                                      }
                                                  ),
                                                )
                                            ),
                                            ...
                                            Container(
                                                child: SizedBox(
                                                  ...
                                                  child: MaterialButton(
                                                      ...
                                                      onPressed: (){
                                                        decreaseQuantity(productId: item.id, userId: globalUserId);
                                                      }
                                                  ),
                                                ),
                                        ),
                            ...
                            child: Container(
                              ...
                              child: Column(
                                ...
                                children: [
                                  ...
                                  Text(
                                    "${formatNumber(sumOfAllForBascket(orderItems).toInt())}" + " تومان" ,
                                    textAlign: TextAlign.right,
                                    textDirection: TextDirection.rtl,
                                    style: TextStyle(
                                  ...
                                  Row(
                                      children: <Widget>[
                                        Expanded(
                                          child: Container(
                                            margin: EdgeInsets.fromLTRB(20, 20, 5, 1),
                                            child: MaterialButton(
                                                padding: EdgeInsets.fromLTRB(40, 15, 20, 15),
                                                shape: RoundedRectangleBorder(
                                                    borderRadius: BorderRadius.all(Radius.circular(20))
                                                ),
                                                color: Color(0xFF00A316),
                                                child: Row(
                                                    children: [
                                                      Text("پرداخت",
                                        ...
                                        Expanded(
                                          child: Container(
                                            margin: EdgeInsets.fromLTRB(5, 20, 20, 1),
                                            child: MaterialButton(
                                                padding: EdgeInsets.fromLTRB(45, 15, 20, 15),
                                                shape: RoundedRectangleBorder(
                                                    borderRadius: BorderRadius.all(Radius.circular(20))
                                                ),
                                                color: Color(0xFFA32D00),
                                                child: Row(
                                                    children: [
                                                      Text("کنسل",
                                                        style: TextStyle(color: Colors.white),textAlign: TextAlign.center,),
